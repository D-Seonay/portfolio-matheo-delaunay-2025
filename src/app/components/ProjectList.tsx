...
export default function ProjectList() {
  ...
  const lang = pathname.startsWith("/en") ? "en" : "fr"; // Détecte la langue
  ...
  useEffect(() => {
    fetch(`/api/projects?lang=${lang}`)
      .then((res) => res.json())
      .then((data: ApiResponse<Project[]>) => setProjects(data.data));
  }, [lang]);
  ...
  return (
    <div className="relative min-h-screen flex flex-col">
      ...
      <main className="flex-grow flex flex-col items-center justify-center text-center text-white px-6">
        ...
        >
          Your Vision, Our <span className="text-yellow-400">Expertise</span> — <br /> Let's Build Together
        </motion.h1>
        <motion.p
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <motion.button
          ...
          className="mt-6 bg-yellow-400 text-black px-6 py-3 rounded-lg shadow-lg hover:bg-yellow-300"
        >
          Start Today ↗
        </motion.button>
      </main>
